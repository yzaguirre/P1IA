...
do
	...
	# xidel aatrox.html -e 'css("#counterpicks-list.picks-list")'| sed 's/[ \t\n]*//' | sed '/^$/d' | tail -n +2 | head -n -1 | awk 'NR % 7 != 0' | awk 'NR % 6 != 0' | awk 'NR % 5 != 0' | awk 'NR % 4 != 0' | less
	# xidel aatrox.html -e 'css("#goodagainst-list.picks-list")'| sed 's/[ \t\n]*//' | sed '/^$/d' | tail -n +2 | head -n -1 | awk 'NR % 7 != 0' | awk 'NR % 6 != 0' | awk 'NR % 5 != 0' | awk 'NR % 4 != 0' | less
	# xidel aatrox.html -e 'css("#duopicks-list.picks-list")'| sed 's/[ \t\n]*//' | sed '/^$/d' | tail -n +2 | head -n -1 | awk 'NR % 5 != 0' | awk 'NR % 4 != 0' | less
	xidel ${name_nosuff}.html -e 'css("#counterpicks-list.picks-list")' | sed 's/[ \t\n]*//' | sed '/^$/d' | tail -n +2 | awk 'NR % 7 != 0' | awk 'NR % 6 != 0' | awk 'NR % 5 != 0' | awk 'NR % 4 != 0' | head -n -1 > ${name_nosuff}.bad_against.txt
	xidel ${name_nosuff}.html -e 'css("#goodagainst-list.picks-list")' | sed 's/[ \t\n]*//' | sed '/^$/d' | tail -n +2 | head -n -1 | awk 'NR % 7 != 0' | awk 'NR % 6 != 0' | awk 'NR % 5 != 0' | awk 'NR % 4 != 0' > ${name_nosuff}.good_against.txt
	xidel ${name_nosuff}.html -e 'css("#duopicks-list.picks-list")' | sed 's/[ \t\n]*//' | sed '/^$/d' | tail -n +2 | head -n -1 | awk 'NR % 5 != 0' | awk 'NR % 4 != 0' > ${name_nosuff}.good_with.txt
	rm ${name_nosuff}.html
done < "$filename"